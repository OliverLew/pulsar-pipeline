...
! open(unit=11,file="pulsar.txt",status='unknown')
! Read(11,*) Tage,Dist,Edot,alpha!Tage in s; Dist in pc; Edot in mec^2 
! close(11)
...
write(0,*) t/Tage
End Do
! open(11,file='difelec.txt',status="unknown")
do ie=1,Edim
...
if(n>NMAX) then
 write(0,*) " tridag ... n>NMAX"
 return
endif
if(b(1)==0.) then
 write(0,*) " tridag ... rewrite equations"
 return
endif
bet=b(1)
u(1)=r(1)/bet
do j=2,n
 ...
 bet=b(j)-a(j)*gam(j)
 if(bet==0.) then
    write(0,*) a(11),b(11),c(11)
    write(0,*) a(500),b(500),c(500)
    write(0,*) " tridag ... tridag failed"
    stop
    return
 endif
